...
| `vol_dryup_ratio` | `mean(Vol B+1..yesterday) / mean(Vol A..B)` | Whether the pullback thinned. Lower is more constructive — selling that dries up is the shape that precedes a turn. Elevated volume in a retracement selects for distribution just as readily. Today is outside the window: it is the session this ratio has to arm, so letting its volume into the average lets a strong turn fail its own gate. |
| `fib_turn_vol_ratio` | `Vol_0 / mean(Vol B+1..yesterday)` | Today's volume against the pullback it would be ending. Deliberately not rel_volume: that 50-session window spans the impulse leg, so it asks the most of the turn where the advance was best confirmed. Null until the pullback is three sessions old. |
| `is_fib_reversal_bar` | `close>open AND (C-L)/(H-L)>=0.60 AND fib_turn_vol_ratio>=1.5 AND L<=level(0.500)` | The event, as opposed to the location. Range travelled and held, on volume expanding against the pullback, having reached into the zone. |
